...
const GraphicsCoinChild = ({
  ...
  const drawBaseHand = useCallback((graphics, x, y) => {
    graphics.clear();
    ...
    // Draw palm with modern rounded square shape
    graphics.setFillStyle({ color: handBase });
    graphics.roundRect(x - 30, y - 5, 60, 50, 15); // Rounded rectangle instead of ellipse
    graphics.fill();
    
    // Palm shadow for depth
    graphics.setFillStyle({ color: handShadow });
    graphics.roundRect(x - 32, y - 3, 56, 46, 13);
    graphics.fill();
    
    // Palm highlight with gradient effect
    graphics.setFillStyle({ color: handHighlight, alpha: 0.7 });
    graphics.roundRect(x - 25, y - 8, 50, 40, 12);
    graphics.fill();
    
    // Modern geometric palm lines
    graphics.setStrokeStyle({ color: handAccent, width: 2 });
    graphics.moveTo(x - 20, y - 2);
    graphics.lineTo(x - 5, y + 30);
    graphics.stroke();
    
    graphics.moveTo(x - 12, y + 2);
    graphics.lineTo(x + 2, y + 35);
    graphics.stroke();
    
    // Add modern geometric patterns
    graphics.setFillStyle({ color: handGlow, alpha: 0.3 });
    graphics.circle(x - 15, y + 10, 8);
    graphics.fill();
    graphics.circle(x + 10, y + 15, 6);
    graphics.fill();
    
    // Draw fingers with modern stylized design and animation
    const fingers = [
      { 
        name: 'pinky', 
        x: x - 42, 
        ...
        segments: [
          { x: x - 42 + fingerWave1 * 0.3, y: y - 8, width: 10, height: 18 },
          { x: x - 42 + fingerWave1 * 0.6, y: y - 20, width: 9, height: 15 },
          { x: x - 42 + fingerWave1 * 0.9, y: y - 30, width: 8, height: 12 }
        ]
      },
      { 
        name: 'ring', 
        ...
        segments: [
          { x: x - 27 + fingerWave2 * 0.3, y: y - 12, width: 12, height: 21 },
          { x: x - 27 + fingerWave2 * 0.6, y: y - 26, width: 11, height: 18 },
          { x: x - 27 + fingerWave2 * 0.9, y: y - 38, width: 10, height: 15 }
        ]
      },
      { 
        name: 'middle', 
        ...
        segments: [
          { x: x - 12 + fingerWave3 * 0.3, y: y - 18, width: 13, height: 24 },
          { x: x - 12 + fingerWave3 * 0.6, y: y - 35, width: 12, height: 21 },
          { x: x - 12 + fingerWave3 * 0.9, y: y - 50, width: 11, height: 18 }
        ]
      },
      { 
        name: 'index', 
        ...
        segments: [
          { x: x + 15 + fingerWave4 * 0.3, y: y - 12, width: 12, height: 21 },
          { x: x + 15 + fingerWave4 * 0.6, y: y - 26, width: 11, height: 18 },
          { x: x + 15 + fingerWave4 * 0.9, y: y - 38, width: 10, height: 15 }
        ]
      }
    ];
    ...
    fingers.forEach(finger => {
      finger.segments.forEach((segment, index) => {
        ...
        graphics.setFillStyle({ color: handBase });
        graphics.roundRect(segment.x - segment.width/2, segment.y - segment.height/2, segment.width, segment.height, 8);
        graphics.fill();
        
        // Shadow for depth
        graphics.setFillStyle({ color: handShadow });
        graphics.roundRect(segment.x - segment.width/2 + 1, segment.y - segment.height/2 + 1, segment.width - 1, segment.height - 1, 7);
        graphics.fill();
        
        // Highlight with modern gradient effect
        graphics.setFillStyle({ color: handHighlight, alpha: 0.6 });
        graphics.roundRect(segment.x - segment.width/2 + 2, segment.y - segment.height/2 + 2, segment.width * 0.7, segment.height * 0.5, 6);
        graphics.fill();
        
        // Modern joint lines
        if (index < finger.segments.length - 1) {
          graphics.setStrokeStyle({ color: handAccent, width: 1 });
          graphics.moveTo(segment.x - segment.width/2, segment.y + segment.height/2);
          graphics.lineTo(segment.x + segment.width/2, segment.y + segment.height/2);
          graphics.stroke();
        }
    ...
    // Draw thumb with modern stylized design
    graphics.setFillStyle({ color: handBase });
    graphics.roundRect(x + 22, y + 2, 22, 30, 12); // Rounded rectangle thumb
    graphics.fill();
    
    graphics.setFillStyle({ color: handShadow });
    graphics.roundRect(x + 20, y + 4, 20, 28, 10);
    graphics.fill();
    
    // Thumb highlight
    graphics.setFillStyle({ color: handHighlight, alpha: 0.6 });
    graphics.roundRect(x + 24, y + 6, 16, 22, 8);
    graphics.fill();
    ...
    fingers.forEach(finger => {
      ...
      graphics.roundRect(tipSegment.x - tipSegment.width/2, tipSegment.y - tipSegment.height/2 + 1, tipSegment.width * 0.8, 8, 6);
      graphics.fill();
      
      // Fingertip highlight
      graphics.setFillStyle({ color: fingertipHighlight, alpha: 0.8 });
      graphics.roundRect(tipSegment.x - tipSegment.width/2 + 1, tipSegment.y - tipSegment.height/2 + 2, tipSegment.width * 0.5, 5, 4);
      graphics.fill();
    });
    
    // Draw modern hand outline
    graphics.setStrokeStyle({ color: handAccent, width: 3 });
    graphics.roundRect(x - 30, y - 5, 60, 50, 15);
    graphics.stroke();
    
    // Add modern tech-style patterns
    graphics.setFillStyle({ color: handGlow, alpha: 0.4 });
    for (let i = 0; i < 8; i++) {
      const dotX = x + (Math.random() - 0.5) * 40;
      const dotY = y + 5 + (Math.random() - 0.5) * 25;
    ...
    graphics.moveTo(x - 25, y + 5);
    graphics.lineTo(x + 25, y + 5);
    graphics.stroke();
  ...
  const drawCoverHand = useCallback((graphics, x, y) => {
    graphics.clear();
    ...
    // Draw fingers with modern stylized design (covering pose) and animation
    const fingers = [
      { 
        name: 'pinky', 
        x: x - 42, 
        ...
        segments: [
          { x: x - 42 + coverFingerWave1 * 0.3, y: y - 5, width: 10, height: 15 },
          { x: x - 42 + coverFingerWave1 * 0.6, y: y - 15, width: 9, height: 12 },
          { x: x - 42 + coverFingerWave1 * 0.9, y: y - 23, width: 8, height: 10 }
        ]
      },
      { 
        name: 'ring', 
        ...
        segments: [
          { x: x - 27 + coverFingerWave2 * 0.3, y: y - 8, width: 12, height: 18 },
          { x: x - 27 + coverFingerWave2 * 0.6, y: y - 20, width: 11, height: 15 },
          { x: x - 27 + coverFingerWave2 * 0.9, y: y - 30, width: 10, height: 12 }
        ]
      },
      { 
        name: 'middle', 
        ...
        segments: [
          { x: x - 12 + coverFingerWave3 * 0.3, y: y - 12, width: 13, height: 21 },
          { x: x - 12 + coverFingerWave3 * 0.6, y: y - 28, width: 12, height: 18 },
          { x: x - 12 + coverFingerWave3 * 0.9, y: y - 42, width: 11, height: 15 }
        ]
      },
      { 
        name: 'index', 
        ...
        segments: [
          { x: x + 15 + coverFingerWave4 * 0.3, y: y - 8, width: 12, height: 18 },
          { x: x + 15 + coverFingerWave4 * 0.6, y: y - 20, width: 11, height: 15 },
          { x: x + 15 + coverFingerWave4 * 0.9, y: y - 30, width: 10, height: 12 }
        ]
      }
    ];
    ...
    // Draw thumb with modern stylized design
    graphics.setFillStyle({ color: handBase });
    graphics.roundRect(x + 22, y + 7, 22, 27, 12);
    graphics.fill();
    
    graphics.setFillStyle({ color: handShadow });
    graphics.roundRect(x + 20, y + 9, 20, 25, 10);
    graphics.fill();
    
    // Thumb highlight
    graphics.setFillStyle({ color: handHighlight, alpha: 0.6 });
    graphics.roundRect(x + 24, y + 11, 16, 20, 8);
    graphics.fill();
    ...
    fingers.forEach(finger => {
      ...
      // Modern fingertip design
      graphics.setFillStyle({ color: fingertipColor });
      graphics.roundRect(tipSegment.x - tipSegment.width/2, tipSegment.y - tipSegment.height/2 + 1, tipSegment.width * 0.8, 6, 6);
      graphics.fill();
      
      // Fingertip highlight
      graphics.setFillStyle({ color: fingertipHighlight, alpha: 0.8 });
      graphics.roundRect(tipSegment.x - tipSegment.width/2 + 1, tipSegment.y - tipSegment.height/2 + 2, tipSegment.width * 0.5, 4, 4);
      graphics.fill();
    });
  ...
  const drawThumbsUpHand = useCallback((graphics, x, y) => {
    graphics.clear();
    ...
    // Draw fingers (closed/fist position) with modern stylized design and animation
    const fingers = [
      { 
        name: 'pinky', 
        x: x - 33, 
        ...
        segments: [
          { x: x - 33 + thumbWave1 * 0.2, y: y + 12, width: 9, height: 18 },
          { x: x - 33 + thumbWave1 * 0.4, y: y + 3, width: 8, height: 15 },
          { x: x - 33 + thumbWave1 * 0.6, y: y - 5, width: 7, height: 12 }
        ]
      },
      { 
        name: 'ring', 
        ...
        segments: [
          { x: x - 18 + thumbWave2 * 0.2, y: y + 8, width: 10, height: 21 },
          { x: x - 18 + thumbWave2 * 0.4, y: y - 3, width: 9, height: 18 },
          { x: x - 18 + thumbWave2 * 0.6, y: y - 14, width: 8, height: 15 }
        ]
      },
      { 
        name: 'middle', 
        ...
        segments: [
          { x: x - 3 + thumbWave3 * 0.2, y: y + 5, width: 12, height: 24 },
          { x: x - 3 + thumbWave3 * 0.4, y: y - 8, width: 11, height: 21 },
          { x: x - 3 + thumbWave3 * 0.6, y: y - 22, width: 10, height: 18 }
        ]
      },
      { 
        name: 'index', 
        ...
        segments: [
          { x: x + 18 + thumbWave4 * 0.2, y: y + 8, width: 10, height: 21 },
          { x: x + 18 + thumbWave4 * 0.4, y: y - 3, width: 9, height: 18 },
          { x: x + 18 + thumbWave4 * 0.6, y: y - 14, width: 8, height: 15 }
        ]
      }
    ];
    ...
    // Draw thumb (thumbs up position) with modern stylized segments and animation
    const thumbWave = Math.sin(fingerAnimationTime * 4) * 5; // More energetic thumb animation
    const thumbSegments = [
      { x: x + 37 + thumbWave * 0.2, y: y - 8, width: 18, height: 27 },
      { x: x + 37 + thumbWave * 0.5, y: y - 23, width: 15, height: 24 },
      { x: x + 37 + thumbWave * 0.8, y: y - 38, width: 12, height: 21 }
    ];
    
    thumbSegments.forEach((segment, index) => {
      graphics.setFillStyle({ color: handBase });
      graphics.roundRect(segment.x - segment.width/2, segment.y - segment.height/2, segment.width, segment.height, 10);
      graphics.fill();
      
      graphics.setFillStyle({ color: handShadow });
      graphics.roundRect(segment.x - segment.width/2 + 1, segment.y - segment.height/2 + 1, segment.width - 1, segment.height - 1, 9);
      graphics.fill();
      
      graphics.setFillStyle({ color: handHighlight, alpha: 0.6 });
      graphics.roundRect(segment.x - segment.width/2 + 2, segment.y - segment.height/2 + 2, segment.width * 0.7, segment.height * 0.5, 8);
      graphics.fill();
    ...
    graphics.roundRect(thumbTip.x - thumbTip.width/2, thumbTip.y - thumbTip.height/2 + 1, thumbTip.width * 0.8, 9, 8);
    graphics.fill();
    
    graphics.setFillStyle({ color: 0xfde047, alpha: 0.8 }); // Light yellow highlight
    graphics.roundRect(thumbTip.x - thumbTip.width/2 + 1, thumbTip.y - thumbTip.height/2 + 2, thumbTip.width * 0.5, 6, 6);
    graphics.fill();
    ...
    // Add enhanced animation effect - golden glow around thumb
    graphics.setFillStyle({ color: 0xfbbf24, alpha: 0.4 });
    graphics.roundRect(x + 25, y - 35, 30, 45, 20);
    graphics.fill();
    
    graphics.setFillStyle({ color: 0xfbbf24, alpha: 0.2 });
    graphics.roundRect(x + 20, y - 40, 40, 55, 25);
    graphics.fill();
    
    // Add modern sparkle effect
    graphics.setFillStyle({ color: 0xffffff, alpha: 0.9 });
    graphics.circle(x + 45, y - 30, 3);
    graphics.fill();
    graphics.circle(x + 30, y - 15, 2.5);
    graphics.fill();
    graphics.circle(x + 52, y - 8, 2);
    graphics.fill();
  ...
  return (
    <>
      ...
      <pixiGraphics draw={(graphics) => showLauncher ? drawThumbsUpHand(graphics, 300, 250) : drawBaseHand(graphics, 300, 250)} />
...
const AdvancedGraphicsCoinGame = () => {
...
export default AdvancedGraphicsCoinGame;
